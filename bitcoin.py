...
def computeDelta(wt, X, Xi):
    ...
    def similarity(a, b):
      ...
      return sum([(a_value - a_mean) * (b_value - b_mean)for a_value, b_value in zip(a,b)])  \
                /(len(a) * np.std(a) * np.std(b))

    numerator = 0
    denominator = 0

    x_length = range(0, len(X) - 1)

    for i in xrange(0, len(Xi.index)) :
      sim = math.exp(wt * similarity(X.iloc[x_length].values, Xi.iloc[i,x_length].values))
      numerator = numerator + (Xi.iloc[i]['Yi'] * sim)
